...
SKM is a package developed to be used in the R ecosystem. A stable version is avaible at The Comprehensive R Archive Network (CRAN), the main repository for R packages, so you can install SKM from a R terminal with the following command.
...
It is of special interest the _kernelize_ function in SKM since this is the first package which allows the use of kernels with different machine learning algorithms as a new approach of working with complex non-linear data and high dimensional data.
