...
One of the major advantages of the use of sparse kernels is data dimensionality reduction since the number of independent variables (columns) is reduced at most the size as the number of records (rows) without losing much information. This is useful when working with high dimensional data where number of columns is considerably greater than number of rows since there is less data model training is quicker.
...
Note that all versions implements random forest using under the hood the randomForestSRC package and in the last implementation no tunining is performed.
...
The folowing examples are going to use the iris dataset too for simplicity but you have to keep in mind that kernels are recommended to be used with high dimensional data.
